...
export class ListPostulationComponent implements OnInit{
  ...
  verifierPostulation(id: number) {
    ...
    if (token) {
      ...
      this.http.post(`http://127.0.0.1:8081/api/verifier/${id}`, {}, { headers })
        .subscribe(
          (response: any) => {
            console.log('Postulation vérifiée avec succès :', response);
            // Actualiser la liste des postulations après la vérification
            this.getPostulation();
            this.router.navigate(['/listPostulation']);
          },
          error => {
  ...
  refuserPostulation(id: number) {
    ...
    if (token) {
      ...
      this.http.post(`http://127.0.0.1:8081/api/incomplet/${id}`, {}, { headers })
        .subscribe(
          (response: any) => {
